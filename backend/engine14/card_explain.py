...
CARD_CATALOG: Dict[str, Dict[str, str]] = {
    ...
    "entry_state": {
        ...
        "spec": (
            ...
            "the same bucket.\n"
            "- Spot (Entry): SPX cash price used as the reference price for "
            "the entry date. If the requested entry date has no printed bar "
            "(weekend, holiday, or a future-dated run before the close is "
            "available), the card shows the most recent available close and "
            "is stamped with an ‘as of YYYY-MM-DD (market closed)’ caption in "
            "amber — treat this as a last-known-spot, not a live quote. All "
            "the wing-distance % and EM-multiple numbers on this strip are "
            "computed against this spot.\n"
    ...
    "outcome_distribution": {
        ...
        "spec": (
            ...
            "of credit collected) and closed for a clean win — MAE during "
            "the trade never approached stop territory.\n"
            "- **Full Collect**: rolled to (or near) expiration and ended "
            "positive (capped at 95%+ of credit for the strict case, but any "
            "positive expiry P&L lands here) without ever reaching stop "
            "territory. A calm win.\n"
            "- **White Knuckle**: a win that was *not* calm — exited "
            "positive (either on the profit target or at expiry) but the "
            "intraday/EOD MAE during the trade reached stop territory "
            "(defined as MAE ≤ the larger of -50% of credit or -50% of the "
            "stop-loss distance, whichever is more adverse). Functionally a "
            "win, but a stressful one the desk should flag when sizing.\n"
            "- **Stop Out**: triggered the loss stop (debit ≥ stop_loss_pct "
            "× credit) OR rolled to expiry and finished below zero without "
            "hitting the stop rule — both are treated as realized losses.\n"
